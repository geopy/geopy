...
from geopy.util import decode_page, logger
...
class Google(Geocoder):
    ...
    def __init__(self, api_key=None, domain='maps.googleapis.com',
        ...
        """
        super(Google, self).__init__(format_string)

        warn('geopy.geocoders.google: The `geocoders.google.Google` geocoder '
            'uses the older "V2" API and is deprecated and may be broken at '
            'any time. A geocoder utilizing the "V3" API is available at '
            '`geocoders.googlev3.GoogleV3` and will become the default in a '
            'future version. See RELEASES file and http://goo.gl/somDT for '
            'usage information.',
            DeprecationWarning
        )
        ...
        if not api_key:
            raise ValueError(
                ...
                "deprecated and may not work after March 2013 or September "
                "2013."
            )
    ...
    def parse_json(self, page, exactly_one=True):
        ...
        if len(places) == 0:
            # Got empty result. Parse out the status code and raise an
            # error if necessary.
    ...
    def check_status_code(status_code):
        ...
        if status_code == 400:
            ...
            raise GeocoderResultError(
                "Unkown error (Server returned status 500)"
            )
        elif status_code == 601:
            raise GQueryError("An empty lookup was performed")
        elif status_code == 602:
            raise GQueryError(
                "No corresponding geographic location could be found for "
                "the specified location, possibly because the address is "
                "relatively new, or because it may be incorrect."
            )
        elif status_code == 603:
            raise GQueryError(
                "The geocode for the given location could be returned due to "
                "legal or contractual reasons"
            )
        elif status_code == 610:
            raise GBadKeyError(
                "The api_key is either invalid or does not match the domain "
